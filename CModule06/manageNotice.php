...
<body>
    ...
    <main>
        <section class=""><div class="inner df fc g50" style="margin-top: 80px;">
            <h1 class="t df jc">Manage Notices</h1>
            <div class="df fc g10">
                <!-- <div class="df je g30">
                    <div class="df g10">
                ...
                <div class="bw1 bx2">
                    <form action="noticeCtrl.php" class="df bx2 cp bw1 jsb ac" style="height: 50px; padding: 10px;">
                        ...
                        <div class="df ac g20">
                            <select class="btn1 b10 f12 bs cm df jc" style="width: 80px;" name="type">
                                <option selected value="일반">일반</option>
                                <option value="이벤트">이벤트</option>
                            </select>
                            <input type="text" class="f16" style="width: 500px;" name="text" placeholder="input here">
                        </div>
                        <div class="df g20 ac">
                            <p class="cg b">"Date" automattcaliy insert</p>
                            <div class="df g5">
                                <button class="btn1 bm cw1 hd f16 b5 b">추가</button>
                            </div>
                        </div>
                    </form>
                    <ul class="df fc">
                        <?php $d = DB::fetchAll("SELECT * FROM Notice ORDER BY date DESC")?>
                        <?php foreach($d as $key => $value):?>
                        <form action="noticeCtrl.php" class="df bx2 cp bw1 jsb ac" style="height: 50px; padding: 10px;">
                            <input type="text" name="action" value="update" hidden>
                            <input type="text" name="idx" value="<?=$value->idx?>" hidden>
                            <div class="df ac g20">
                                <select class="btn1 b10 f12 bs cm df jc" style="width: 80px;" name="type">
                                    <option <?=$value->type == "일반" ? 'selected' : ''?> value="일반">일반</option>
                                    <option <?=$value->type == "이벤트" ? 'selected' : ''?> value="이벤트">이벤트</option>
                                </select>
                                <input type="text" class="f16" style="width: 500px;" name="text" value="<?=$value->text?>">
                            </div>
                            <div class="df g20 ac">
                                <p class="cg b"><?=$value->date?></p>
                                <div class="df g5">
                                    <button class="btn1 hd f16 b5 cb bw3 b">수정</button>
                                    <a class="btn1 hd f16 bm cw1 b5 b" href="./noticeCtrl.php?action=delete&idx=<?=$value->idx?>">삭제</a>
                                </div>
                            </div>
                        </form>
                        <?php endforeach; ?>
