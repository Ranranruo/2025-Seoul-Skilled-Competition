...
const modal = () => {
    ...
    const render = async () => {
        ...
        // 주문영역
        data = await fetch('./productCtrl.php?action=fetchAll').then(data => data.json());
    ...
    const getHtml = (data, count = null, isBuy = false) => {
        ...
        <li class="product bx2 b10 oh" style="width: 250px; opacity: ${isBuy ? '.5' : '1'}" data-idx='${data.idx}' draggable="true">
            ...
            ${data.sale != 0 ? `<div class="f10 bs cm b5 btn1 bx2">SALE</div>` : ''}
            </div>
    ...
    const getPrice = (price, sale) => {
        ...
        if(sale == 0) return `<div class="price df g5 b cm"><div class="origin">${intP.toLocaleString()}</div></div>`;
...
const products = () => {
    ...
    const render = async () => {
        ...
        $$products.forEach(el => {
            ...
            filteredData = filteredData.toSorted((a, b) => (b.sale != 0) - (a.sale != 0));
    ...
    const getHtml = (data) => {      
        ...
        <div class="bx2 b10 oh" style="width: 250px;">
            ...
            <div class="df fc g10" style="padding: 15px;">
                <div class="w df fc"><p class="s b f18">${data.name}</p><p class="s cg f14">${data.description}</p></div>
                ${getPrice(data.price, data.sale)}
                <div class="df jsb"><button class="hd bm cw1 btn1 b5 f14">구매하기</button><a href="cartCtrl.php?action=insert&memberId=${memberId}&productId=${data.idx}" class="df jc ac hd cb bw3 btn1 b5 f14">장바구니담기</a></div>
            </div>
        </div>
        `;
    }
